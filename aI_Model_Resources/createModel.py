...
def predict_tweet(tweet_list):
    pickle_off = open("naive.pkl","rb")
    model_1 = pickle.load(pickle_off)
    processed_tweets = preprocess(tweet_list)
    df = pd.DataFrame(processed_tweets, columns=['text','links','emojis','labels'])
    x = [item for item in df['text']]
    # vector = CountVectorizer()
    # vector.fit(x)
    # vector = CountVectorizer(vocabulary=vector.vocabulary_)
    # x_tweets = vector.transform(x)
