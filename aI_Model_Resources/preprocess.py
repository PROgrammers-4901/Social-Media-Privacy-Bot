...
def remove_urls(tweet_list):
    ...
    for temp in range(0,len(tweet_list)):
        tweet_links.append(regex.findall(r'https?://\S+',tweet_list[temp]))

        tweet_list[temp] = (urls.sub(r'',tweet_list[temp]))
...
def remove_emojis(tweet_list):
    ...
    emojis = regex.compile("["
                           ...
                           "]+", flags=regex.UNICODE)
    for temp in range(0,len(tweet_list)):
        if emojis.search(tweet_list[temp]) == None:
            tweet_emojis.append(False)
        else:
            tweet_emojis.append(True)
        tweet_list[temp] = emojis.sub(r'',tweet_list[temp])
...
def remove_upper(tweet_list):
    ...
    for num in range(0,len(tweet_list)):
        tweet_list[num] = tweet_list[num].lower()
...
def tokens_and_punct(tweet_list):
    ...
    new_list = [regex.sub(r'@\w+', "" , y) for y in tweet_list]
